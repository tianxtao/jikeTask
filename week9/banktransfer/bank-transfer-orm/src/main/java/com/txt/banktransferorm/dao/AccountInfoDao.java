package com.txt.banktransferorm.dao;
...
public interface AccountInfoDao {
    //查询账户信息
    @Select("select * from account_info where account_no = #{accountNo} and account_type = #{accountType};")
    AccountInfo queryAccountInfo(@Param("accountNo") String accountNo, @Param("accountType") int accountType);

    // 查询账户余额
    @Select("select account_balance from account_info where account_no = #{accountNo} and account_type = #{accountType};")
    double queryAccountBalance(@Param("accountInfo") AccountInfo accountInfo, @Param("accountType") int accountType);

    //从账户表中划走金额
    @Update("update account_info set account_balance=account_balance - #{amount} where account_no=#{accountNo} and account_type = #{accountType}; ")
    int subtractAccountBalance(@Param("amount") double amount, @Param("accountNo") String accountNo,@Param("accountType") int accountType);

    // 给账户增加金额
    @Update("update account_info set account_balance=account_balance + #{amount} where account_no=#{accountNo} and account_type = #{accountType}; ")
    int addAccountBalance(@Param("amount") double amount, @Param("accountNo") String accountNo, @Param("accountType") int accountType);

    // 添加冻结金额
    @Insert("insert into account_temp(account_name, account_no, amount, create_time, account_type) values(#{accountName},#{accountNo},#{amount},#{createTime}, #{accountType});")
